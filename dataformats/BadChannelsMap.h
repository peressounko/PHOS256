#ifndef BADCHANNELSMAP_H
#define BADCHANNELSMAP_H
...
class BadChannelsMap
{
 public:
  ...
  void AddBadChannel(int channelID) { mBadCells.set(channelID - OFFSET); } // set bit to true

  /// \brief Mark channel as good
  /// \param channelID Absolute ID of the channel
  /// Setting channel as good.
  void SetChannelGood(int channelID) { mBadCells.set(channelID - OFFSET, false); }
  ...
  bool IsChannelGood(int channelID) const { return !mBadCells.test(channelID - OFFSET); }
  ...
  void GetHistogramRepresentation(TH2* h) const;
 ...
 private:
  static constexpr int NCHANNELS = 256; ///< Number of channels starting from 1 (4*64*56+1

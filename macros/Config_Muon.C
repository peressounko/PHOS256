...
void Config()
{
  ...
  GenMuon * gen = new GenMuon(); //Acceptance theta, phi
  gen->SetSecondTrigger();

  gSim->SetGenerator(gen);


  //Construct PHOS
  Phos * ph = new Phos(1.,0.) ; //Distance to crystal, angle
  gSim->SetPHOS(ph);
...
//  geant4->RegisterPhysics(new G4EmLivermorePolarizedPhysics());
